...
if __name__ == '__main__':
    ...
    with open('kittimots_train_coco.dat', 'wb') as f:
        pickle.dump(train_records, f)
        print("Generated kittimots_train_coco.dat", flush=True)

    val_records = generate_dict(VALIDATION)
    print("\nVal %d" % len(val_records))
    with open('kittimots_val_coco.dat', 'wb') as f:
        pickle.dump(val_records, f)
        print("Generated kittimots_val_coco.dat", flush=True)

    test_records = generate_dict(TEST)
    print("\nTest %d" % len(test_records))
    with open('kittimots_test_coco.dat', 'wb') as f:
        pickle.dump(test_records, f)
        print("Generated kittimots_test_coco.dat", flush=True)
